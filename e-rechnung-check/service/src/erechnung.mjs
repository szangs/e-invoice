// E-Rechnung: Erkennung, Datenextraktion und formale Pflichtfeld-Prüfung.
// Eigenständige Portierung von einvoice/src/lib/erechnung.ts (ohne Next.js /
// Prisma / Pfad-Aliase), für den KoSIT-Prüfdienst auf dem vServer.
//
// Formate: ZUGFeRD/Factur-X (PDF mit eingebettetem CII-XML), XRechnung
// (reines XML, Syntax UBL oder UN/CEFACT CII), normales PDF.
// Formale Prüfung: Kernfelder nach EN 16931 / §14 UStG. Die vollständige
// Schematron-/KoSIT-Validierung übernimmt kosit.mjs.
...
function parseCii(root) {
  ...
  const lines = asArray(tx?.IncludedSupplyChainTradeLineItem).map((li) => ({
    name: val(li?.SpecifiedTradeProduct?.Name) ?? '—',
    quantity: val(li?.SpecifiedLineTradeDelivery?.BilledQuantity),
    lineTotal: num(
      li?.SpecifiedLineTradeSettlement?.SpecifiedTradeSettlementLineMonetarySummation?.LineTotalAmount,
    ),
  }))
...
function parseUbl(inv) {
  ...
  const vat = asArray(supplier?.PartyTaxScheme)
    .map((r) => val(r?.CompanyID))
    .find(Boolean)

  const lines = asArray(inv?.InvoiceLine).map((li) => ({
...
async function extractEmbeddedXml(buffer) {
  try {
...
/** Zentrale Analyse einer eingehenden Datei. */
export async function analyzeInvoiceFile(buffer, mimeType, fileName) {
  const isXml =
