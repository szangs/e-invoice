...
// vollständig abgelaufenes Kalenderjahr des EIGENEN Mandanten — siehe
// api/audit/period-close (Stefan 2026-08-27: vorher im Betreiber-Cockpit,
// systemweit für alle Mandanten gleichzeitig — jetzt je Mandant, vom
// Mandanten-Administrator selbst ausgelöst). Einmal abgeschlossen,
// unveränderlich (kein erneutes Abschließen); Belege aus abgeschlossenen
// Jahren gelten ab sofort als gesperrt (lib/auditClosure.ts).
...
export function PeriodClosurePanel({ years }: { years: PeriodRow[] }) {
  ...
  async function close(year: number) {
    ...
    setBusyYear(year)
    setMsg('')
    const res = await fetch('/api/audit/period-close', {
      method: 'POST',
              ...
              <td className="dp-td">
                {y.closed ? (
                  <div className="flex gap-2">
                    <a className="btn-secondary !px-2 !py-1 text-xs" href={`/api/audit/period-close/${y.year}/certificate`} target="_blank" rel="noreferrer">
                      Zertifikat
                    </a>
                    <a className="btn-secondary !px-2 !py-1 text-xs" href={`/api/audit/period-close/${y.year}/zip`}>
                      ZIP-Archiv
                    </a>
                  </div>
                ) : y.closable ? (
